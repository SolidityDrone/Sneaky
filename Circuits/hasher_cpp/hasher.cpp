...
void Hasher_11_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void Hasher_11_run(uint ctx_index,Circom_CalcWit* ctx);
...
uint get_total_signal_no() {return 8131;}

uint get_number_of_components() {return 772;}

uint get_size_of_input_hashmap() {return 256;}

uint get_size_of_witness() {return 710;}

uint get_size_of_constants() {return 42;}
...
for (uint i = 0; i < 1; i++) {
std::string new_cmp_name = "unhashedBits";
...
Fr_lt(&expaux[0],&lvar[0],&circuitConstants[17]); // line circom 13
while(Fr_isTrue(&expaux[0])){
{
uint cmp_index_ref = 0;
{
...
PFrElement aux_dest = &lvar[0];
// load src
Fr_add(&expaux[0],&lvar[0],&circuitConstants[1]); // line circom 13
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
Fr_lt(&expaux[0],&lvar[0],&circuitConstants[17]); // line circom 13
...
Hasher_11_create(1,0,ctx,"main",0);
Hasher_11_run(0,ctx);
}
